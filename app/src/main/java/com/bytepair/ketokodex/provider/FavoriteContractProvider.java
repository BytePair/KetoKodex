...
public class FavoriteContractProvider extends ContentProvider {
    ...
    public int delete(@NonNull Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            ...
            case FAVORITE_WITH_ID:
                ...
                favoritesDeleted = db.delete(FavoriteContract.FavoriteEntry.TABLE_NAME, "_id=?", new String[]{id});
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        }
    ...
    public int update(@NonNull Uri uri, ContentValues values, String selection,
        ...
        final SQLiteDatabase db = mFavoriteDbHelper.getWritableDatabase();
        int match = sUriMatcher.match(uri);
        // Keep track of the number of updated favorites
        int favoritesUpdated;

        switch (match) {
            case FAVORITES:
                favoritesUpdated = db.update(FavoriteContract.FavoriteEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            case FAVORITE_WITH_ID:
                if (selection == null) selection = FavoriteContract.FavoriteEntry._ID + "=?";
                else selection += " AND " + FavoriteContract.FavoriteEntry._ID + "=?";
